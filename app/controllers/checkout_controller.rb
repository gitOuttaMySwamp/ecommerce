...
class CheckoutController < ApplicationController
  before_action :set_cart, only: [:invoice]
  ...
  def invoice
    @cart.each { |piece| @id_array << piece.id }
    @cart.each { |piece| @subtotal += (piece.size.price + piece.detail.price) }
  ...
  def success
    ...
    session_p["display_items"].each do |item|
      ...
      if item["custom"]["name"].to_s.eql?("GST") ||
         item["custom"]["name"].to_s.eql?("PST")
